...
- 資訊呈現方式：原則 6 要求熟悉工具的用戶能儘快得到分析結果。最快的結果就是一次互動就拿到詳細分析結果或開始角色扮演。因此：
  1. 從歡迎到拿到詳細分析結果之間，不能有強制性的問題。
  2. 但在這前提之下，必須不嚇跑不熟悉工具的用戶（原則 7）
  3. 也要讓不熟悉工具的用戶可以確認蒐集的細節是否正確（原則 3）

  目前的作法是
  1. 開始角色扮演或分析前，顯示摘要細節與目標（原則 3 與 7），但盡量簡短（原則 6）
  2. 分析採用倒三角形，並在最後的詳細分析之前使用視覺線索（分隔線，「以下是細節」）引導不熟悉的用戶跳過
...
- 元資訊不得含有任何開發者個人資料，否則當用戶剛好是開發者時，如果底層 AI 將紀錄去個人化，可能反而暴露出用戶是哪個開發者。（原則 1）
